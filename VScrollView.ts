...
/**
 * 虚拟滚动列表（从 Bingo 工程抽来，已去掉玩法绑定）。
 * 等大小 / 不等高、网格、惯性、下拉刷新、上拉加载、分页。
 * 子项点击与合批分层见 VScrollViewItem。
 */
...
export class VirtualScrollView extends Component {
  ...
  @property({
    displayName: '启用分页滚动',
    tooltip: '启用后，每次滑动会自动停在指定数量的项目位置',
    visible(this: VirtualScrollView) {
      return this.useVirtualList;
    },
  })
  ...
  @property({
    displayName: '===每页项目数',
    tooltip: '每次滑动停止时对齐的项目数量（例如：3表示每滑过3个项目就停下）',
    range: [1, 20, 1],
    visible(this: VirtualScrollView) {
      return this.useVirtualList && this.enablePagination;
    },
  })
  ...
  @property({
    displayName: '===分页动画时长',
    tooltip: '自动对齐到分页位置的动画时长（秒）',
    range: [0.1, 2, 0.1],
    visible(this: VirtualScrollView) {
      return this.useVirtualList && this.enablePagination;
    },
  })
  public paginationDuration: number = 0.3;

  @property({
    displayName: '===翻页速度阈值',
    tooltip: '滑动速度超过此值（像素/秒）时会翻到下一页，否则对齐到最近页',
    range: [100, 3000, 100],
    visible(this: VirtualScrollView) {
      return this.useVirtualList && this.enablePagination;
    },
  })
  public pageFlipVelocityThreshold: number = 800;
  ...
  public setTotalCount(count: number) {
    ...
    // 只调用 _updateVisible(true)，它内部会调用 _layoutSlots，避免重复
    this._updateVisible(true);
  }
  ...
  private _scrollToPosition(targetPos: number, animate = false, onComplete?: () => void) {
  ...
  public scrollToIndex(index: number, animate = false): Promise<void> {
    return new Promise(resolve => {
      const clampedIndex = math.clamp(index | 0, 0, Math.max(0, this.totalCount - 1));
      let targetPos = 0;
      if (this.useDynamicSize) {
        targetPos = this._prefixPositions[clampedIndex] || 0;
      } else {
        const line = Math.floor(clampedIndex / this.gridCount);
        targetPos = line * (this.itemMainSize + this.spacing);
      }
      if (!this._isVertical()) {
        targetPos = -targetPos;
      }
      this._scrollToPosition(targetPos, animate, resolve);
    });
  }
  ...
  public getPageCount(): number {
    if (!this.enablePagination || !this.useVirtualList || this.useDynamicSize) return 0;
    const safeItemsPerPage = Math.max(1, Math.round(this.itemsPerPage));
    return Math.max(1, Math.ceil(this.totalCount / safeItemsPerPage));
  }
  ...
  public getCurrentPageIndex(): number {
    ...
    const itemsPerPage = Math.max(1, Math.round(this.itemsPerPage));
    const pos = this._getContentMainPos();
    const currentFloatIndex = this._isVertical() ? pos / stride : -pos / stride;
    const pageFloat = currentFloatIndex / itemsPerPage;
    const pageCount = this.getPageCount();
    if (pageCount <= 0) return 0;
    return math.clamp(Math.round(pageFloat), 0, pageCount - 1);
  }

  /**
   * 按页码跳转（0 基）
   * @param pageIndex 目标页码
   * @param animate 是否使用滚动动画
   */
  public scrollToPage(pageIndex: number, animate = false) {
    if (!this.enablePagination || !this.useVirtualList || this.useDynamicSize) {
      console.warn('[VScrollView] scrollToPage 仅在等大小虚拟列表且启用分页时可用');
      return;
    }
    const pageCount = this.getPageCount();
    if (pageCount <= 0) return;
    const safeItemsPerPage = Math.max(1, Math.round(this.itemsPerPage));
    const targetPage = math.clamp(pageIndex | 0, 0, pageCount - 1);
    const targetIndex = targetPage * safeItemsPerPage;
    const stride = this.itemMainSize + this.spacing;
    let targetPos = targetIndex * stride;
    if (!this._isVertical()) targetPos = -targetPos;
  ...
  private _stopTouchEvent(e?: EventTouch) {
    if (!this.blockParentScroll || !e || !this._scrollEnabled) return;
   ...
   * 开关用户滑动交互（不影响代码调用的滚动）
   */
  public setScrollEnabled(enabled: boolean) {
   ...
   * 对齐到最近的分页位置
   */
  private DangQianYeIndex: number = 0;
  private _snapToPage() {
    const currentPos = this._getContentMainPos();
    const stride = this.itemMainSize + this.spacing;
    ...
    // 计算当前位置对应的索引（浮点数）
    let currentFloatIndex: number;
    if (this._isVertical()) {
      currentFloatIndex = currentPos / stride;
    } else {
      currentFloatIndex = -currentPos / stride;
    }

    // 计算当前所在的页（浮点数）
    const currentPageFloat = currentFloatIndex / this.itemsPerPage;
    ...
    if (absVelocity > this.pageFlipVelocityThreshold) {
      // 速度够大，根据方向翻页
      if (this._isVertical()) {
        // 纵向：正速度向上翻页，负速度向下翻页
        if (velocity > 0) {
      ...
      // 速度不够，对齐到最近的页
      targetPageIndex = Math.round(currentPageFloat);
    }
    ...
    targetPageIndex = math.clamp(targetPageIndex, 0, maxPage);
    //console.log(`[VScrollView] 切换至当前页面=${currentPageIndex} 目标=${targetPageIndex} 速度=${velocity.toFixed(1)} 页面=${pageCount}`);
    if (currentPageIndex == targetPageIndex) {
      this.DangQianYeIndex = currentPageIndex;
    } else {
      this.DangQianYeIndex = targetPageIndex;
    }
    this.node.parent.emit("event_yeshu_SerchOutLimits", this.DangQianYeIndex);
    // 计算目标位置
    const targetIndex = targetPageIndex * this.itemsPerPage;
    let targetPos: number;
    if (this._isVertical()) {
      targetPos = targetIndex * stride;
    } else {
      targetPos = -targetIndex * stride;
    }
    ...
    const minBound = Math.min(this._boundsMin, this._boundsMax);
    const maxBound = Math.max(this._boundsMin, this._boundsMax);
    targetPos = math.clamp(targetPos, minBound, maxBound);
    ...
    // 使用原有的 _scrollToPosition 方法（animate = true）
    this._scrollToPosition(targetPos, true);
  }
  ...
  private _layoutSlots(firstIndex: number, forceRender: boolean) {
    ...
    // console.log(`[VScrollView] _layoutSlots 调用\n    firstIndex: ${firstIndex}\n    forceRender: ${forceRender}\n    调用栈:\n      ${stack}`);
